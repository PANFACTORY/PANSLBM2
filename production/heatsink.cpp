...
int main(int argc, char** argv) {
    ...
    double Pr = 6.0, Ra = 1e3, nu = 0.1, L = 4.0, tem0 = 0.0, qn = 1.0e-2, alphamax = 1.0e4, beta = 1.0, betamax = 16.0;
    ...
    for (int k = 1, cnt = 1; k <= nk; k++) {
        ...
        //********************Check convergence********************
        if (MyRank == 0) {
            std::cout << "\r" << std::fixed << std::setprecision(6) << k << " " << f << " " << g << " " << td << " " << ti << " " << dsmax  << " (" << imax << "," << jmax << ") " << qf << " " << qg << " " << mnd << " " << beta << std::endl;
        }
        if (dsmax < 0.01 || k == nk) {
