...
private:
    ...
    void MMA<T>::UpdateVariables(std::vector<T>& _xk, T _f, const std::vector<T>& _dfdx, const std::vector<T>& _g, const std::vector<std::vector<T> >& _dgdx){       
        ...
        std::vector<T> b_buffer(this->m, T()), b(this->m);
		for(int i = 0; i < this->m; i++){
			for(int j = 0; j < this->n; j++){
        ...
        std::vector<T> deltillambda_buffer(this->m), deltillambda(this->m);
        ...
        std::vector<T> dy(this->m);
        T dz = T();
        std::vector<T> dlambda_buffer(this->m), dlambda(this->m);
        ...
        for(int l = 0; eps > 1.0e-7; l++){
            ...
            if(this->n > this->m){
                std::vector<T> A_buffer((this->m + 1)*(this->m + 1), T()), A((this->m + 1)*(this->m + 1), T());
                ...
                std::vector<T> B_buffer(this->m, T()), B(this->m + 1);
                for(int ii = 0; ii < this->m; ii++){
    ...
    T MMA<T>::KKTNorm(
        ...
        T norm_buffer = T(), norm = T();

        //----------Get parameters----------
        std::vector<T> plambda(this->n);
        std::vector<T> qlambda(this->n);
        std::vector<T> g_buffer(this->m, T()), g(this->m, T());
        for(int j = 0; j < this->n; j++){
            plambda[j] = _p0[j];
            qlambda[j] = _q0[j];
            for(int i = 0; i < this->m; i++){
