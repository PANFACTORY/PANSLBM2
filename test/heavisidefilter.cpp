...
int main(int argc, char** argv) {
    ...
    std::vector<double> fv = HeavisideFilter::GetFilteredVariable(pf, R, beta, v);
    VTKXMLExport file(pf, "result/heavisidefilter");
    file.AddPointData(pf, "v", [&](int _i, int _j, int _k) { return v[pf.Index(_i, _j, _k)]; });
    file.AddPointData(pf, "fv", [&](int _i, int _j, int _k) { return fv[pf.Index(_i, _j, _k)]; });
#ifdef _USE_MPI_DEFINES
    MPI_Finalize();
#endif
    return 0;
