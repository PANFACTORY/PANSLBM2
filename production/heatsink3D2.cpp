...
int main(int argc, char** argv) {
    ...
    //********************Parameters********************
    int lx = 81, ly = 161, lz = 81, nt = 10000, dt = 100, nitr = 500, nb = 100, resultid = 0;
    double Pr = 6.0, Ra = 2.5e4, nu = 0.1, tem0 = 0.0, qn0 = 1.0e-2, alphamax = 1.0e4;
    double qf = 1e-2, qfmax = 1e2, qg = 1e0, movelimit = 0.2, weightlimit = 0.05, R = 2.4, eps = 1.0e-6, s0 = 0.0;

    int mx = 3*(lx - 1)/4 + 1, my = 3*(ly - 1)/4 + 1, mz = 3*(lz - 1)/4 + 1, bx = (lx - 1)/10 + 1, by = (ly - 1)/40 + 1, bz = (lz - 1)/10 + 1;
    ...
    double *heatsource = new double[pg.nxyz];
    ...
    for (int i = 0; i < pf.nx; ++i) {
        for (int j = 0; j < pf.ny; ++j) {
            for (int k = 0; k < pf.nz; ++k) {
                ...
                if ((i + pf.offsetx) < mx && by <= (j + pf.offsety) && (j + pf.offsety) < my + by && (k + pf.offsetz) < mz) {
                    s[idx] = s0;
                    snm1[idx] = s0;
                } else if ((i + pf.offsetx) < bx && (j + pf.offsety) < by && (k + pf.offsetz) < bz) {
                    s[idx] = 0.0;
                    snm1[idx] = 0.0;
                }
            }
    ...
    auto filterweight = [=](int _i1, int _j1, int _k1, int _i2, int _j2, int _k2) {
        if (_i1 < mx && by <= _j1 && _j1 < my + by && _k1 < mz && _i2 < mx && by <= _j2 && _j2 < my + by && _k2 < mz) {
    ...
    for (int itr = 1, cnt = 1; itr <= nitr; itr++) {
        ...
        for (int t = 1; t <= nt; t++) {
            AD::MacroBrinkmanCollideNaturalConvectionHeatSource(pf, rho, ux, uy, uz, alpha, nu, pg, tem, qx, qy, qz, diffusivity, heatsource, gx, gy, gz, tem0, true, gi);
            if (t%dt == 0) {
                if (MyRank == 0) {
        ...
        for (int t = 1; t <= nt; t++) {
            AAD::MacroBrinkmanCollideNaturalConvectionHeatCompliance(
                pf, rho, ux, uy, uz, irho, iux, iuy, iuz, imx, imy, imz, alpha, nu, 
                pg, tem, item, iqx, iqy, iqz, diffusivity, heatsource,
                gx, gy, gz, true, igi
            );
            if (t%dt == 0) {
                if (MyRank == 0) {
        ...
        for (int i = 0; i < pf.nx; ++i) {
            for (int j = 0; j < pf.ny; ++j) {
                for (int k = 0; k < pf.nz; ++k) {
                    if ((i + pf.offsetx) < bx && (j + pf.offsety) < by && (k + pf.offsetz) < bz) {
                        f_buffer += tem[pf.Index(i, j, k)];
                    }
                } 
            }
        }
        ...
        AAD::SensitivityBrinkmanDiffusivity(pg, dfdss.data(), ux, uy, uz, imx, imy, imz, dads, tem, item, iqx, iqy, iqz, gi, igi, diffusivity, dkds);
        ...
        for (int i = 0; i < pf.nx; ++i) {
            for (int j = 0; j < pf.ny; ++j) {
                for (int k = 0; k < pf.nz; ++k) {
                    int idx = pf.Index(i, j, k);
                    dfds[idx] = ((i + pf.offsetx) < mx && by <= (j + pf.offsety) && (j + pf.offsety) < my + by && (k + pf.offsetz) < mz) ? dfds[idx] : 0.0;
                    dgds[idx] = ((i + pf.offsetx) < mx && by <= (j + pf.offsety) && (j + pf.offsety) < my + by && (k + pf.offsetz) < mz) ? dgds[idx] : 0.0;
                }
            }
        }
...
#ifdef _USE_MPI_DEFINES
            VTKXMLExport file(pf, "result/heatsink3D_" + std::to_string(resultid));
...
#else
            VTKExport file("result/heatsink3D_" + std::to_string(resultid) + ".vtk", lx, ly, lz);
    ...
    delete[] gi;    delete[] igi;   delete[] heatsource;
    std::chrono::system_clock::time_point end = std::chrono::system_clock::now();
    if (MyRank == 0) {
        std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count() << std::endl;
    }
