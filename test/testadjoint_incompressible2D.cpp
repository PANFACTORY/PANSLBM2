...
#include "../src/equation/navierstokes_incompressible.h"
#include "../src/equation/adjointnavierstokes_incompressible.h"
#include "vtkexport.h"
...
int main() {
    ...
    //--------------------Direct analyze--------------------
    for (int t = 0; t < nt; t++) {
        NSin::UpdateMacro(particle, &rho[nx*ny*t], &ux[nx*ny*t], &uy[nx*ny*t]);       //  Update macroscopic values
        NSin::Collision(nu, particle, &rho[nx*ny*t], &ux[nx*ny*t], &uy[nx*ny*t]);     //  Collision
        particle.Stream();                              //  Stream
        for (int j = 0; j < ny; j++) {
            if (0.35*ny < j && j < 0.65*ny) {
                double uj = -u0*(j - 0.35*ny)*(j - 0.65*ny)/(0.0225*ny*ny);
                particle.SetUIn(0, j, uj, 0.0);
                particle.SetRhoIn(nx - 1, j, rho0, 0.0);
            }
        }                                               //  Boundary condition (inlet)
        NSin::ExternalForceBrinkman(particle, alpha, alpha);     //  External force by Brinkman model

        if (t > 0 && NSin::CheckConvergence(particle, epsdu, &ux[nx*ny*t], &uy[nx*ny*t], &ux[nx*ny*(t - 1)], &uy[nx*ny*(t - 1)])) {
            std::cout << "\tt = " << t << "\t";
            tmax = t;
            break;
        }
    ...
    //--------------------Invert analyze--------------------
    for (int i = 0; i < nx*ny; i++) {
        NSin::InitialCondition(i, particle, 0.0, 0.0, 0.0);
    }                                                   //  Set initial condition
    for (int t = tmax; t >= 0; t--) {
        ANSin::UpdateMacro(particle, &rho[nx*ny*t], &ux[nx*ny*t], &uy[nx*ny*t], qho, vx, vy);     //  Update macroscopic values
        ANSin::Collision(nu, particle, &ux[nx*ny*t], &uy[nx*ny*t], qho, vx, vy);                  //  Collision
        particle.iStream();                             //  Stream
        for (int j = 0; j < ny; j++) {
            if (0.35*ny < j && j < 0.65*ny) {
                particle.SetiUIn(0, j);
                particle.SetiRhoIn(nx - 1, j);
            }
        }                                               //  Boundary condition (inlet)
        ANSin::ExternalForceBrinkman(particle, alpha, alpha, &rho[nx*ny*t], &ux[nx*ny*t], &uy[nx*ny*t]); //  External force by Brinkman model
        ANSin::UpdateSensitivity(particle, &ux[nx*ny*t], &uy[nx*ny*t], sensitivity);              //  Update sensitivity
    }
    ANSin::UpdateMacro(particle, rho, ux, uy, qho, vx, vy);     //  Update macroscopic values
    ...
    //--------------------Export result--------------------
    VTKExport file("result/adjointin.vtk", nx, ny);
