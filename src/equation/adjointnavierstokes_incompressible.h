//*****************************************************************************
//  Title       :   src/equation/adjointnavierstokes_incompressible.h
//  Author      :   Tanabe Yuta
//  Date        :   2021/01/13
...
namespace PANSLBM2 {
    ...
    namespace ANS {
        ...
        void UpdateMacro(P<T>& _particle, T* _rho, T* _ux, T* _uy, T* _q, T* _vx, T* _vy) {
            ...
            for (int i = 0; i < _particle.np; i++) {
                ...
                for (int j = 0; j < P<T>::nc; j++) {
                    ...
                    _q[i] += _particle.ft[j][i]*P<T>::ei[j];
        ...
        void Collision(T _viscosity, P<T>& _particle, T* _ux, T* _uy, T* _q, T* _vx, T* _vy) {
            ...
            for (int i = 0; i < _particle.np; i++) {
                for (int j = 0; j < P<T>::nc; j++) {
                    T feq = _q[i] + 3.0*(_vx[i]*P<T>::cx[j] + _vy[i]*P<T>::cy[j]);
        ...
        void ExternalForceBrinkman(P<T>& _particle, T* _alphax, T* _alphay, T* _rho, T* _ux, T* _uy) {
            ...
            for (int i = 0; i < _particle.np; i++) {
                ...
                for (int j = 0; j < P<T>::nc; j++) {
                    _particle.ft[j][i] -= 3.0*_particle.dx*(_alphax[i]*mx*P<T>::cx[j]/(1.0 + _particle.dx*_alphax[i]) + _alphay[i]*my*P<T>::cy[j]/(1.0 + _particle.dx*_alphay[i]));
                }
            }
        }
