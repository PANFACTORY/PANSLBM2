...
int main() {
    ...
    for (int i = 0; i < nx*ny; i++) {
        NSin::InitialCondition(i, particle, 1.0, 0.0, 0.0);
    ...
    for (int t = 0; t < tmax; t++) {
        NSin::UpdateMacro(particle, rho, u, v);     //  Update macroscopic values
        NSin::Collision(nu, particle, rho, u, v);   //  Collision
        particle.Stream();                          //  Stream
        for (int i = 0; i < nx; i++) {
            particle.SetUIn(i, ny - 1, u0, 0.0);
        }                                           //  Boundary condition (inlet)
        
        if (t%1000 == 0) {
            std::cout << t/1000 << std::endl;
            VTKExport file("result/nsin" + std::to_string(t/1000) + ".vtk", nx, ny);
