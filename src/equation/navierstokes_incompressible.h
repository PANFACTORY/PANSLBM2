//*****************************************************************************
//  Title       :   src/equation/navierstokes_incompressible.h
//  Author      :   Tanabe Yuta
//  Date        :   2021/01/13
    ...
    namespace NSin {
        ...
        void Collision(T _viscosity, P<T>& _particle, T* _rho, T* _ux, T* _uy) {
            ...
            for (int i = 0; i < _particle.np; i++) {
                for (int j = 0; j < P<T>::nc; j++) {
                    ...
                    _particle.ftp1[j][i] = (1.0 - omega)*_particle.ft[j][i] + omega*P<T>::ei[j]*(_rho[i] + 3.0*ciu + 4.5*ciu*ciu - 1.5*uu);
                }
            }
        }
        ...
        void InitialCondition(int _i, P<T>& _particle, T _rho, T _ux, T _uy) {
            ...
            for (int j = 0; j < P<T>::nc; j++) {
                ...
                _particle.ft[j][_i] = P<T>::ei[j]*(_rho + 3.0*ciu + 4.5*ciu*ciu - 1.5*uu);
            }
        }
    }
}