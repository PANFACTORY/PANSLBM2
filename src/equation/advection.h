...
namespace PANSLBM2 {
    namespace AD2 {
        ...
        void InitialCondition(int _i, P<T>& _particle, T _temperature, T _ux, T _uy) {
            ...
            for (int j = 0; j < P<T>::nc; j++) {
                T ciu = P<T>::ci[j][0]*_ux + P<T>::ci[j][1]*_uy;
    ...
    namespace NS2 {
        ...
        void ExternalForceNaturalConvection(T _gx, T _gy, T _temperature0, P<T>& _particlef, Q<T>& _particleg) {
            assert(P<T>::nd == 2 && assert(Q<T>::nd == 2););
            for (int i = 0; i < _particlef.np; i++) {
                ...
                for (int j = 0; j < P<T>::nc; j++) {
                    _particlef.ft[j][i] += 3.0*_particlef.dx*P<T>::ei[j]*(Q<T>::ci[j][0]*_gx + Q<T>::ci[j][1]*_gy)*(temperature - _temperature0);
                }
            }
        }
    }
